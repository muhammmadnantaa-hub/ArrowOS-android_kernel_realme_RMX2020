...
index 3bbe62a1e..c9d9eef6b 100644
--- a/fs/Makefile
+++ b/fs/Makefile
...
index 5cb6e4fea..98245c380 100644
--- a/fs/namei.c
+++ b/fs/namei.c
@@ -39,6 +39,9 @@
...
@@ -2192,10 +2354,21 @@ static int link_path_walk(const char *name, struct nameidata *nd)
 	for(;;) {
 		u64 hash_len;
 		int type;
+#ifdef CONFIG_KSU_SUSFS_SUS_PATH
...
@@ -2221,6 +2394,24 @@ static int link_path_walk(const char *name, struct nameidata *nd)
...
@@ -2387,7 +2578,9 @@ static inline int lookup_last(struct nameidata *nd)
 {
...
@@ -3278,15 +3471,55 @@ static int lookup_open(struct nameidata *nd, struct path *path,
...
@@ -3412,6 +3645,9 @@ static int do_last(struct nameidata *nd,
 	struct path path;
 	int error;
...
@@ -3731,12 +3967,19 @@ static struct file *path_openat(struct nameidata *nd,
 	return file;
 }
...
@@ -3744,6 +3987,25 @@ struct file *do_filp_open(int dfd, struct filename *pathname,
...
index d772fe46e..6fe8cdf4a 100644
--- a/fs/namespace.c
+++ b/fs/namespace.c
@@ -27,10 +27,40 @@
...
@@ -100,6 +130,25 @@ static inline struct hlist_head *mp_hash(struct dentry *dentry)
...
+#endif
 ...
 {
 	int res;
@@ -120,6 +169,35 @@ static int mnt_alloc_id(struct mount *mnt)
...
+#endif
 	...
 	if (mnt_id_start > id)
@@ -136,6 +214,19 @@ static int mnt_alloc_group_id(struct mount *mnt)
 {
 	int res;
...
@@ -154,6 +245,17 @@ static int mnt_alloc_group_id(struct mount *mnt)
...
+#endif
 	...
 	if (mnt_group_start > id)
 		mnt_group_start = id;
@@ -201,13 +303,31 @@ static void drop_mountpoint(struct fs_pin *p)
 	mntput(&m->mnt);
 }
...
@@ -1039,7 +1159,17 @@ vfs_kern_mount(struct file_system_type *type, int flags, const char *name, void
 	if (!type)
...
@@ -1065,6 +1195,15 @@ vfs_kern_mount(struct file_system_type *type, int flags, const char *name, void
...
+
 	...
 	unlock_mount_hash();
@@ -1093,8 +1232,52 @@ static struct mount *clone_mnt(struct mount *old, struct dentry *root,
...
@@ -1146,6 +1329,15 @@ static struct mount *clone_mnt(struct mount *old, struct dentry *root,
...
+
 	...
 	unlock_mount_hash();
@@ -1870,6 +2062,7 @@ struct mount *copy_tree(struct mount *mnt, struct dentry *dentry,
 	p = mnt;
...
@@ -1901,7 +2094,7 @@ struct mount *copy_tree(struct mount *mnt, struct dentry *dentry,
 				goto out;
 			lock_mount_hash();
...
@@ -2364,6 +2557,27 @@ static int do_loopback(struct path *path, const char *old_name,
...
+
 out2:
 	unlock_mount(mp);
 out:
@@ -2963,6 +3177,15 @@ long do_mount(const char *dev_name, const char __user *dir_name,
 	else
...
+#endif
 dput_out:
 	path_put(&path);
 	return retval;
@@ -3040,6 +3263,10 @@ struct mnt_namespace *copy_mnt_ns(unsigned long flags, struct mnt_namespace *ns,
 	struct mount *old;
 	struct mount *new;
 	int copy_flags;
+#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
...
@@ -3059,6 +3286,14 @@ struct mnt_namespace *copy_mnt_ns(unsigned long flags, struct mnt_namespace *ns,
...
+#endif
 	...
 	if (IS_ERR(new)) {
 		namespace_unlock();
@@ -3095,6 +3330,29 @@ struct mnt_namespace *copy_mnt_ns(unsigned long flags, struct mnt_namespace *ns,
...
+
 	...
 	if (rootmnt)
@@ -3635,3 +3893,38 @@ const struct proc_ns_operations mntns_operations = {
...
index c62a87ee3..f3437ed78 100644
...
index 403cbb12a..7c3fad2cb 100644
--- a/fs/proc/cmdline.c
+++ b/fs/proc/cmdline.c
@@ -4,8 +4,18 @@
 #include <linux/proc_fs.h>
...
index 96fc70225..7c5583125 100644
--- a/fs/proc/fd.c
+++ b/fs/proc/fd.c
@@ -12,6 +12,9 @@
 #include <linux/fs.h>
...
index f5c0fc7ba..c444ac50a 100644
...
+#endif
 ...
 #include <asm/elf.h>
 #include <asm/tlb.h>
@@ -345,6 +348,10 @@ static void show_vma_header_prefix(struct seq_file *m,
...
+
 ...
 {
@@ -359,8 +366,17 @@ show_map_vma(struct seq_file *m, struct vm_area_struct *vma, int is_pid)
...
index b859aaeec..0c8f54724 100644
...
index e6f4c7b88..9f281557a 100644
--- a/fs/readdir.c
+++ b/fs/readdir.c
@@ -23,6 +23,14 @@
...
index 0fda4b6b8..bbc053d04 100644
--- a/fs/stat.c
+++ b/fs/stat.c
@@ -17,6 +17,9 @@
...
index e6ceb3e87..bdf49a178 100644
--- a/fs/statfs.c
+++ b/fs/statfs.c
@@ -9,6 +9,10 @@
 #include <linux/security.h>
...
index 1ff21c19b..99adc5d71 100644
...
index 936d550a1..056563fff 100644
...
index 9a1cafc8d..a070595a7 100644
--- a/kernel/kallsyms.c
+++ b/kernel/kallsyms.c
...
index 30801ea06..5a76d4255 100644
--- a/kernel/sys.c
+++ b/kernel/sys.c
...
index 1b8f957c2..f37460685 100644
